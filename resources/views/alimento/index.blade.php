...
@section('content')
    <div class="conteiner">
        <div class="app-title">
            <h1>
                <i class="fa fa-edit">Lista de Alimento</i>
            </h1>
    ...
    <div claas="container">
        <div class="tile">
            <div class="tile-body">
                <form class="form-inline" method="POST" action="{{ url('/alimento/pesquisar') }}">
                    @csrf
                    {{-- <div class="col-sm-12"> --}}
                        {{-- <div class="form-grup"> --}}
        ...
        <div class="tile">
            <div class="title-body">
                <div id="no-more-tables">
                        ...
                        <tbody>
                            @foreach ($registros as $registro)
                                <tr>
                                    <td>{{ $registro->id }}</td>
                                    <td>{{ $registro->nome }}</td>
                                    <td>{{ $registro->peso_padrao }}</td>
                                    <td>{{ $registro->carbo }}</td>
                                    <td>{{ $registro->proteina }}</td>
                                    <td>{{ $registro->gordura }}</td>
                                    <td>
                                        <a class="btn btn-info btn-sm" href="{{ url('alimento/alterar', $registro->id) }}"><i
                                                class="fa fa-pencil"></i></a>
                                        <a class="btn btn-danger btn-sm"
                                            href="{{ url('alimento/deletar', $registro->id) }}"><i
                                                class="fa fa-trash"></i></a>
                                        <a class="btn btn-warning btn-sm"
                                            href="{{ url('alimento/consultar', $registro->id) }}"><i
                    ...
                    @endisset
                    <a class="btn btn-sucess btn-lg" href="{{ url('alimento/incluir') }}">Incluir<i
